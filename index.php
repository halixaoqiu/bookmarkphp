<?php
	...
	$stmt = $pdo->prepare("SELECT bookmark_id,bookmark.user_id,title,url,summary,is_public,bookmark.create_time,bookmark.modify_time,nick FROM bookmark LEFT JOIN user ON bookmark.user_id=user.user_id order by bookmark.create_time desc limit 50");
	$stmt->execute();
